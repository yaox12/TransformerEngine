...
class BlockwiseGemmKernel:
    ...
    def _setup_attributes(self):
        ...
        if self.is_dw:
            if self.scale_n_per_tile != self.cta_tile_shape_mnk[1]:
                raise ValueError("scale_n_per_tile must be cta_tile_n")
        else:
    ...
    def __call__(
        ...
        """
        if cutlass.const_expr(len(a.shape) == 2):
            a = cute.make_tensor(
                a.iterator,
                cute.make_layout(
                    (
        ...
        if cutlass.const_expr(len(b.shape) == 2):
            b = cute.make_tensor(
                b.iterator,
                cute.make_layout(
                    (
        ...
        if cutlass.const_expr(len(c.shape) == 2):
            c = cute.make_tensor(
                c.iterator,
                cute.make_layout(
                    (
        ...
        if cutlass.const_expr(len(sfa.shape) == 2):
        ...
        if cutlass.const_expr(len(sfb.shape) == 2):
...
te_dtype_to_cutlass_dtype = {
    tex.DType.kFloat8E4M3: cutlass.Float8E4M3FN,
    tex.DType.kFloat32: cutlass.Float32,
    tex.DType.kBFloat16: cutlass.BFloat16,
}
...
def blockwise_gemm_sm100(
    a: Float8BlockwiseQTensorStorage,
    transa: bool,
    ...
    if transa:
        a_data = a._columnwise_data
        a_scale_inv = a._columnwise_scale_inv
    else:
        a_data = a._rowwise_data
        a_scale_inv = a._rowwise_scale_inv
    if transb:
        b_data = b._rowwise_data
        b_scale_inv = b._rowwise_scale_inv
    else:
    ...
    acc_dtype = cutlass.Float32
    c_dtype = te_dtype_to_cutlass_dtype[out_dtype]
...
def blockwise_grouped_gemm_sm100(
    a_list: list[Float8BlockwiseQTensorStorage],
    transa: bool,
    b_list: list[Float8BlockwiseQTensorStorage],
    transb: bool,
    ...
    for s in range(num_stream_used):
        events[s].record(streams[s])
    # wait for all compute streams to finish
    for s in range(num_stream_used):
        current_stream.wait_event(events[s])
